...
export class RequestCombo1Page implements OnInit {
  ...
  ngOnInit() {
    ...
    se.gf.googleAnalytionCustom('add_to_cart',{item_category:'requestcombo' , item_name: se.bookCombo.HotelName, item_id: se.bookCombo.HotelCode, start_date: se.cin, end_date: se.cout, value: Number(se.bookCombo.ComboRoomPrice ? se.bookCombo.ComboRoomPrice.toString().replace(/\./g, '').replace(/\,/g, '') : 0) ,currency: "VND"});
  }
      ...
      request(options, function (error, response, body) {
        ...
        if (error) {
          ...
          if (data.success) {
            ...
            //se.gf.googleAnalytionCustom('ecommerce_purchase',{item_category:'requestcombo' , item_name: se.bookCombo.ComboTitle, item_id: se.bookCombo.HotelCode, start_date: se.cin, end_date: se.cout,number_of_rooms: (se.searchhotel.roomnumber ?se.searchhotel.roomnumber : 1),value: se.bookCombo.titleComboShort+'|'+se.bookCombo.HotelCode+'|'+se.cin+'|'+se.cout+'|'+se.adult+'|'+se.child });
