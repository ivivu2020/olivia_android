...
export class FlightComboReviewsPage implements OnInit {
  ...
  departfi; returnfi; childrendisplay = 0; adulsdisplay = 0;
  ...
  ngOnInit() {
    ...
    //         se.checkvalue(se.listDepart[0], 0);
    ...
    //         se.checkvalue(se.listReturn[0], 1);
  ...
  checkElement(object) {
    ...
    object.forEach(element => {
      if (element && element.MealTypeRates[0].RoomId == se.bookcombodetail.roomId && !element.MSGCode) {
        el = element;
        //Check room còn allotment hay không để chạy tool giữ vé
        if (element.MealTypeRates[0].Status == 'AL' && element.MealTypeRates[0].AvailableNo > 0) {
          se.hasAllotment = true;
        }
      }
    })
    ...
    if (!el) {
      ...
      if (arr && arr.length > 0) {
        el = arr[0];
        //Check room còn allotment hay không để chạy tool giữ vé
        if (el.MealTypeRates[0].Status == 'AL' && el.MealTypeRates[0].AvailableNo > 0) {
          se.hasAllotment = true;
        }
      }
    }
  ...
  callSummaryPrice(element) {
    ...
    if (element && !element.MSGCode) {
      // Giá nhà cung cấp
      this.TravPaxPrices = element.MealTypeRates[0].PriceAvgPlusNet * this.roomnumber * se.TotalNight;
    ...
    this.storage.get('jti').then(jti => {
      if (jti) {
        ...
        var objectFlight = {
          ...
          allowBookComboPaymentOnline: false
        }
        //PDANH 17/07/2019: Nếu có allotment & là vé jetstar thì show thanh toán online
        if (this.isJetstarFlight && this.hasAllotment) {
          objectFlight.allowBookComboPaymentOnline = true;
        }
