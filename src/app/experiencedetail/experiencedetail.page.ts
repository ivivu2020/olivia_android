...
export class ExperienceDetailPage implements OnInit {
  ...
  unlikePlace(item, isItemNearBy){
    ...
    se.storage.get('auth_token').then(auth_token => {
        if (auth_token) {
            ...
            if(item.blogId){
              let urlLikeBlog= C.urls.baseUrl.urlMobile +'/mobile/OliviaApis/RemoveFavouriteBlogByUser';
              se.gf.RequestApi('POST', urlLikeBlog,{authorization: text},{ postId: item.blogId }, 'experiencedetail', 'unlikePlace');
            }  
        }else{
          se.showConfirmLogin("Bạn cần đăng nhập để sử dụng chức năng này.");
        }
  ...
  expandOrHideMap(){
    var se = this;
    se.expandMap = !se.expandMap;
    $('.div-map > iframe')[0].style.cssText = se.expandMap ? "height: 500px" : "height: 100px";
    $('#mapid')[0].style.cssText = se.expandMap ? "height: 500px" : "height: 100px";
  }
}
