...
export class SearchExperienceRegionPage implements OnInit {
  ...
  ionViewWillEnter(){
    var se = this;
    se.clearSearchValue();
    if(se.searchHotel.inputExperienceRegionName && se.searchHotel.deviceLocation.regioncode){
    ...
    else if(se.searchHotel.ItemExperienceRegionRecent && se.searchHotel.ItemExperienceRegionRecent.length>0){
  ...
  calculateDistanceMarker(rest:any, sortColumn){
    ...
    if(!rest.latitude || !rest.longitude){
      ...
      if(sortColumn == 'totalDistance'){
        if(se.searchHotel.deviceLocation && se.searchHotel.deviceLocation.latitude){
  ...
  search(){
    ...
    if(this.itemsRegion.length >0 || (this.itemsRegion.length ==0 && this.textSearchPlace) ){
      ...
      if(itemR.length <=0 && !this.searchHotel.inputExperienceRegionCode && !this.searchHotel.inputExperienceItem){
    ...
    //alert(JSON.stringify(this.searchHotel.inputExperienceItem));
    this.navCtrl.back();
  }

  public async showWarning(msg) { // if(!this.alertCtrl){
