...
export class HotelDetailPage implements OnInit {
  ...
  public hotelMealTypes = [];
  public hotelMealTypesHidden = [];
  public hotelRooms;
  ...
  public async ngOnInit() {
    ...
    se.subscription = se.router.events.subscribe((event) => {
      if (event instanceof NavigationEnd && (event.url === '/app/tabs/hoteldetail/' + se.HotelID)) {
        se.onEnter();
      }
    });
  }
  ...
  getBOD(roomid) {
    ...
    request(options, function (error, response, body) {
      ...
      //check combo xe
      //  if (se.loginuser) {
      //   if ((se.dateofcombo == se.duration+1) && (!se.ischeckBOD && se.checkBODdone)&&se.comboDetailList.length>0 && se.totalAdult<=6&&se.allowbookcombofc) {
...
excuteLoadHotelRoom(data){
      ...
      if(se.hotelRoomClasses.length >0 && se.comboDetail){
        ...
        if(se.comboDetail.comboDetail)
        {
          se.checkRoomDefaultMaxPax(se.comboDetail.comboDetail.roomId, se.hotelRoomClasses).then((check)=>{
            if (se.loadcomplete && (!se.ischeckBOD && se.checkBODdone)&&se.comboDetailList.length>0 && se.hotelRoomClasses.length > 0 && se.totalAdult<=6&&se.hotelRoomClasses.length>0&&se.fcbcar&&se.allowbookcombofx && check) {
              se.ischeckcbcar=true;
            } else {
              se.ischeckcbcar=false;
            }
 ...
 /**
  ...
  async requestCombo(value) {
    ...
    //   if (Number(this.searchhotel.arrchild[i].numage) >= 12) {
    //     co = 1;
    //     break;
    //   }
    // }
    ...
    //Combo vé máy bay
    if (value == 1 && this.usermail && (diffday <= 0)) {
  ...
  choiceFlightDeparture(combodetail: any): any {
    ...
    var timeDiff = Math.abs(date2.getTime() - date1.getTime());
    self.duration = Math.ceil(timeDiff / (1000 * 3600 * 24));
    self.booking.CheckInDate = self.searchhotel.CheckInDate ? self.searchhotel.CheckInDate : this.cin;
    self.booking.CheckOutDate = self.searchhotel.CheckOutDate ? self.searchhotel.CheckOutDate : this.cout,
  ...
  async showDepartureCalendar(combolist) {
    ...
    se.loadcomplete = false;
    se.searchhotel.hotelID = this.HotelID;
    se.searchhotel.roomID = this.RoomID;
    se.searchhotel.ischeckBOD = this.ischeckBOD;
    const modal: HTMLIonModalElement =
      await this.modalCtrl.create({
    ...
    modal.onDidDismiss().then((data: OverlayEventDetail) => {
      if (!data.data !== null) {
        ...
        if (!data.data) {
          //se.storage.get('email').then(email => {
          //if (email) {
          se.ischeckBOD = this.searchhotel.ischeckBOD;
          ...
          se.zone.run(() => {
            ...
            se.cindisplayhr = moment(this.searchhotel.CheckInDate).format('DD/MM');
            se.coutdisplayhr = moment(this.searchhotel.CheckOutDate).format('DD/MM');
          })
  ...
  seemoreroomdetail(value, groupId) {
    ...
    if (value == 1) {
      ...
      // var objs = $('.group-'+groupId+' .cls-hidden');
      // if(objs && objs.length >0){
      //   for(let i = 0; i< objs.length; i++){
      //     $(objs[i]).removeClass('cls-hidden').addClass('cls-visible');
      //   }
      // }
      //group mealtype
