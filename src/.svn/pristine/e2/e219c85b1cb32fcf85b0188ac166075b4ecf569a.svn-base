...
export class HotelListPage implements OnInit {
  ...
  _infiniteScroll;
  memberid: any; hotelId; cindisplayhr = "14/04"; coutdisplayhr = "16/04"; guest = 2; datecin; datecout
    ...
    request(options, function (error, response, body) {
      ...
      if (response.statusCode == 200) {
        ...
        if (searchhotel.chuoi) {
          ...
          se.pushdata(0, se.dataList.length).then(()=>{
            se.listhotels = "";
  ...
  /**Sửa lại bind data
     * PDANH 23/01/2018
     */
  public pushdata(num, len): Promise<any> {
    /**Sửa phần combine điều kiện search
     * PDANH 05/01/2018
     */
    var se = this;
          ...
          if (auth_token) {
            ...
            request(options, function (error, response, body) {
              ...
              se.zone.run(() => {
                ...
                //PDANH 17/06/2019: Sửa lỗi load lại khi back về từ detail
                // se.subscription = se.router.events.subscribe((event) => {
