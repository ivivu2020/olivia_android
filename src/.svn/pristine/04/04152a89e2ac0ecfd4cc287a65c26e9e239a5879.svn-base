        ...
        if(se.searchhotel.inputExperienceItem){
          ...
          // }
            ...
            se.combineDataPlace(se.regionCode,data.data).then((datacombine)=>{
              ...
              //28/12/2019: Sửa lại không mở detail khi click chọn place
              // if(se.searchhotel.inputExperienceItem && se.searchhotel.inputExperienceItem.placeId){
...
/**
  ...
  addDirectionToMap(itemDestination){
    ...
    if(se.searchhotel.inputExperienceItem && !se.searchhotel.inputExperienceItem.placeId){
      ...
      //let inputLon = Math.round(se.searchhotel.inputExperienceItem.longitude *10000000)/10000000;
    ...
    if(showNotice && se.countNotice <3){
      se.showWarning('Xin vui lòng di chuyển bản đồ đến điểm bạn muốn xuất phát!');
      se.countNotice+=1;
    }
    ...
    se.gf.setParams(item,'experienceItem');
      ...
      modal.onDidDismiss().then((data: OverlayEventDetail) => {
       // if (se.sortvalue) {
          ...
          //   })
          //   se.valueGlobal.likePlaceCount = null;
          // }
       // }
      })
    //},200)
  ...
  showDirection(){
    ...
    if(se.currentItemSelect){
      ...
      se.showWarning('Xin vui lòng chọn 1 trải nghiệm!')
    }
  }
  /**
      ...
      se.calculateDistanceMapByCurrentLocationData(se.listSearchDisplay, { latitude:se.map.center.lat(), longitude: se.map.center.lng() }).then((datasort)=>{
        setTimeout(()=>{
          if(datasort && datasort.length >0){
            let firstItem = datasort[0];
            if(firstItem){
              se.currentItemSelect = firstItem;
              if(se.map.getZoom() >= 16){
                se.markerClick(firstItem,firstItem,se.map, true, false);
              }else{
                se.markerIconClick(firstItem,firstItem,se.map, true);
              }
            }
          }
        },150)
